...
func main() {
	...
	entryGenerator, ok := entryGenerators[config.EntryGenerator]
	if !ok {
		bye("entry generator not found: %s", config.EntryGenerator)
	}
	...
	for i := int64(0); i < int64(config.KeyCount); i++ {
		key := entryGenerator.GenerateKey(i)
		value := entryGenerator.GenerateValue(i)
